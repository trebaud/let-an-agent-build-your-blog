---
title: "About"
description: "About this site."
---

This site is built with [Pagewright](https://github.com/your-handle/pagewright), a small
static site generator with one strong opinion: **content is a contract**.

Replace this page with your own. It lives at `content/pages/about.md` — edit the Markdown,
keep the `title` in the frontmatter, and rebuild. To change how it _looks_, you never touch
this file; that's the job of the `site/` layer (or just ask an agent — see the README).

- Add posts to `content/posts/`
- Add pages to `content/pages/`
- Rebrand and restyle in `site/`
